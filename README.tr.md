...
README, ozellikle proje genelindeki mimari kararlarin ve secilen design pattern'lerin kisa gerekcelerini aciklamak uzere hazirlanmistir.
...
- `Feature-based structure` secildi; ability, locomotion ve animation kendi sinirlari icinde gelistirilebilsin, bir feature'daki degisiklik digerini dogrudan kirma riski tasimasin istendi.
- `Dependency Injection (VContainer)` kullanildi; scene tarafindaki UI/input baglantilari ile player prefab icindeki runtime sistemleri acik bir composition yapisiyla birlestirmek ve gizli bagimliliklari azaltmak hedeflendi.
- `Event-driven communication (MessagePipe)` tercih edildi; HUD, animation, input ve execution katmanlari birbirini dogrudan tanimadan haberlesebilsin, boylece feature'lar arasi baglilik gevsek kalsin istendi.
- `Data-driven authoring` benimsendi; yeni bir ability varyanti eklerken once yeni asset uretilmesi, mevcut runtime akisinin ise sadece yeni bir mekanik gerekiyorsa genisletilmesi hedeflendi.
- `Object pooling` zorunlu tutuldu; projectile ve VFX gibi tekrarli objelerde mobil tarafta instantiate/destroy kaynakli GC ve performans maliyetini azaltmak amaclandi.
- `Single asset ability authoring` secildi; mechanic config ve optional modules ayni `AbilityDataSO` altinda tutuldu, boylece authoring hizi artti ve dosya daginigi ile merge karmasasi azaldi.

## Design Patterns

- `Strategy`: her ability'nin cekirdek davranisi secilen `AbilityExecutorSO` ile belirlenir; boylece dash, projectile ve AOE gibi farkli mekanikler ayni runtime akisi icinde degistirilebilir hale gelir.
- `Factory`: `AbilityFactory`, runtime `IAbility` instance uretimini tek noktada toplar; olusturma kurallari daginmaz ve loadout tarafinda standart bir bootstrap akisi korunur.
- `Observer / Pub-Sub`: cooldown, energy, trigger ve loadout bilgileri eventler uzerinden yayilir; bu sayede UI, animation ve gameplay katmanlari birbirine dogrudan baglanmadan senkronize olur.
- `Template / Hook`: optional module yapisi `before/after execute` asamalarina davranis ekler; mevcut executor veya controller akisina mudahale etmeden SFX, VFX veya ek policy'ler baglanabilir.
- `Object Pool`: tekrarli projectile ve VFX instance'lari yeniden kullanilir; bu pattern ozellikle mobil hedefte runtime maliyetini kontrol altinda tutmak icin secilmistir.
